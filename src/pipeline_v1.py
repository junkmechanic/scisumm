...
from utilityFunctions import writeToFile, deleteFiles, trainSvm, predictSvm
...
# Load the Google 1gm tf-idf values
TF_IDF_values = {}
picklefile = DIR['DATA'] + "google-tf-idf-values.pickle"
with open(picklefile, 'rb') as pfile:
    TF_IDF_values = pickle.load(pfile)
...
def processTree(root, ranker, idx):
    # Easiest way to add more features is to have separate functions.
    # Can think about joining them later.
    #trees.append(root)
    if root.word.lower() in newstopwords:
        verb_val = 0.0
    else:
        verb_val = ranker.tfidf_value(idx, root.word)
    verb_tdidf = getGoogleTfidf(root)
    #-------------------------------------
    # For display
    ...
    subj_tfidf = getGoogleTfidf(subj)
    #-------------------------------------
    # For display
    #if subj is not None:
    ...
    obj_tfidf = getGoogleTfidf(obj)
    #-------------------------------------
    # For display
    #if obj is not None:
    #    obj.value += '         ' + str(obj_val)
    #-------------------------------------
    #toreturn = " 1:" + str(verb_val) + " 2:" + str(verb_tdidf)
    #if subj is not None:
    #    toreturn += " 3:" + str(subj_val) + " 4:" + str(subj_tfidf)
    #if obj is not None:
    #    toreturn += " 5:" + str(obj_val) + " 6:" + str(obj_tfidf)
    toreturn = " 2:" + str(verb_tdidf)
    if subj is not None:
        toreturn += " 4:" + str(subj_tfidf)
    if obj is not None:
        toreturn += " 6:" + str(obj_tfidf)
    return(toreturn)


def getGoogleTfidf(node):
    if node is None:
        return 0.0
    else:
        tfidf, num = computeTfidf(node)
        if tfidf == 0.0:
            return 0.0
        else:
            return tfidf / num
...
def computeTfidf(node):
    num = 1
    wrd = node.word.lower()
    if wrd[0] in TF_IDF_values['TF'].keys() and \
       wrd in TF_IDF_values['TF'][wrd[0]].keys():
        tfval = TF_IDF_values['TF'][wrd[0]][wrd]
    else:
        tfval = 0.0
        num = 0
    if wrd[0] in TF_IDF_values['IDF'].keys() and \
       wrd in TF_IDF_values['IDF'][wrd[0]].keys():
        idfval = TF_IDF_values['IDF'][wrd[0]][wrd]
    else:
        idfval = 0.0
        num = 0
    tfidf = tfval * idfval
    for child in node.children:
        value, n = computeTfidf(child)
        tfidf += value
        num += n
    return tfidf, num
